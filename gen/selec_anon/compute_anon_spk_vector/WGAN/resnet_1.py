...
class MLP_D(nn.Module):
    """MLP-based discriminator for 1D embeddings (more suitable than ResNet for this task)."""
    
    def __init__(self, z_dim, size, nfilter=64, nfilter_max=512, **kwargs):
        super().__init__()
        self.size = size  # embedding dimension (192)
        
        # Simple MLP: embedding -> [hidden layers] -> scalar score
        self.fc = nn.Sequential(
            nn.Linear(size, nfilter * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(nfilter * 2, nfilter * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(nfilter * 4, nfilter * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(nfilter * 2, 1)
        )
    ...
    def forward(self, x):
        ...
        Args:
            x: Embedding tensor (batch_size, 192)
            
        Returns:
            Discriminator score (batch_size, 1)
        """
        return self.fc(x)


class MLP_G(nn.Module):
    """MLP-based generator for 1D embeddings."""
    
    def __init__(self, z_dim, size, nfilter=256):
        super().__init__()
        self.z_dim = z_dim
        self.size = size  # embedding dimension (192)
        
        # Simple MLP: latent code -> [hidden layers] -> embedding
        self.fc = nn.Sequential(
            nn.Linear(z_dim, nfilter),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(nfilter, nfilter * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(nfilter * 2, nfilter),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(nfilter, size)
        )
    ...
    def forward(self, z):
        ...
        Returns:
            Generated embeddings (batch_size, size)
        """
        return self.fc(z)
...
class ResNet_D(nn.Module):
    ...
    def __init__(self, z_dim, size, nfilter=64, nfilter_max=512, res_ratio=0.1):
        ...
        # fc_input: map from embedding dimension (size) to image space
        # For 192-dim embeddings: 192 -> 3 * 192 * 192
        self.fc_input = nn.Linear(size, 3 * size * size)
